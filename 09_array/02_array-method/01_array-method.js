...
console.log(`foodList.indexOf('물회', 1) : ${foodList.indexOf('물회', 1)}`);            // 4 // 두번째 것
console.log(`foodList.indexOf('삼겹살') :  ${foodList.indexOf('삼겹살')}`);             // -1 // 존재하지 않으면 -1
...
console.log(`foodList.includes('물회') : ${foodList.includes('물회')}`);                // true // 포함 여부
...
// slice(시작인덱스, 종료인덱스)
// slice(start(인덱스 시작값), end(인덱스 종료값)) -- end 요소의 값은 반환하지 않는다. start <= result < end
...
console.log(`front.splice(3, 1, "React") : ${front.splice(3, 1, "React")}`);    // jQuery // 3번째 배열의 값부터, 1개를 빼고, "React"을 추가한다.
