...
function Student(name, age) { // 생성자 함수는 첫글자 대문자로 설정
...
function Test(name, title, getInfo) {
    this.name = name;
    this.title = title;
    this.getInfo = function() {
        return `${this.name}은(는) ${this.title} 절대 못 해`
    }
}
const maeng = new Test('맹구', '리그 우승')
console.log(maeng.getInfo());
const tot = new Test('톹넘', '아무 우승이나')
console.log(tot.getInfo()); 
